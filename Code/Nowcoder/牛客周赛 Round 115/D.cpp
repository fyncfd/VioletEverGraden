...
* @Create Time: 2025-10-26 19:24:19
...
static inline void VioletEverGraden() {
    ...
    // std::sort(a.begin() + 1, a.end(), [&](pll a, pll b))

    std::sort(a.begin() + 1, a.end(), [&](pll a, pll b) -> bool {
    	if (a.fi == b.fi) {
    		return a.se < b.se;
    	} else {
    		return a.fi < b.fi;
    ...
    while (ok) {
        while (i <= n and a[i].fi <= ans) {
            heap.push(a[i].se);
            i ++;
        }
        while (!heap.empty() and heap.top() < ans) {
            heap.pop();
        }
        if (heap.empty()) {
            ok = false;
