...
* @Create Time: 2025-11-21 19:16:26
...
static inline void VioletEverGraden() {
    ...
    std::reverse(h.begin() + 1, h.end());
    std::reverse(v.begin() + 1, v.end());

    std::vector<int> max1(n + 1, 0); std::vector<int> max2(n + 1, 0);
    rep(i, 1, n, 1) {
        if (v[i] >= max1[i - 1]) {
            max2[i] = max1[i - 1];
            max1[i] = v[i];
    ...
    while (__ --) {
        ...
        int pos = std::lower_bound(h.begin() + 1, h.end(), x) - h.begin();
        int r = pos - 1;
