...
* @Create Time: 2025-07-24 12:08:01
...
static inline void VioletEverGraden() {
    ...
    while (__ --) {
    	...
    	int p = n + 1 - (upper_bound(a.begin() + 1, a.end(), x) - a.begin() + 1);
    	...
    	if (y == 1) {
            int k = (a.back() - x + 1 + 1) / 2;
            std::cout << k << '\n';
            continue;
    	}

        int min = a[y - 2]; int t = n - y + 1;
        int k = std::max((min - x - t + 1) / 2, min - x);
        k = std::max(k, 0);
