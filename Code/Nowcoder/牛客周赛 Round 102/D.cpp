...
* @Create Time: 2025-07-27 19:03:48
...
static inline void VioletEverGraden() {
        ...
        rep(j, 1, 4, 1) {
            rep(c, 0, 1, 1) {
                ...
                if (dp[i - 1][j][c] < INF) {
                    ...
                    dp[i][j][c] = std::min(dp[i][j][c], dp[i-1][j][c] + cost);
                }
                if (j >= 2) {
                    int pre = 1 - c;
                    if (dp[i - 1][j - 1][pre] < INF) {
                        int cost = 0;
                        if (s[i - 1] != p) {
                        	cost = 1;
                        } else {
                        ...
                        dp[i][j][c] = std::min(dp[i][j][c], dp[i - 1][j - 1][pre] + cost);
                    }
                }
            }
        }
    ...
    int ans = std::min(dp[n][4][0], dp[n][4][1]);
