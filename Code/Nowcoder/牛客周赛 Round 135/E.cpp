...
* @Create Time: 2026-03-15 19:29:30
...
static inline void VioletEverGraden() {
    ...
    std::vector<std::array<int, 2>> a(k + 1);
    std::unordered_map<int, int> hash1, hash2;

    for (int i = 1; i <= k; i ++) {
    	std::cin >> a[i][0] >> a[i][1];
    	hash1[a[i][0]] ++; hash2[a[i][1]] ++;
    }

    int sz1 = hash1.size(); int sz2 = hash2.size();
    int cnt = 0;
    for (int i = 1; i <= k; i ++) {
    	if (hash1[a[i][0]] == 1 && hash2[a[i][1]] == 1) {
    		cnt ++;
    	}
    }

    Z ans = (Z(1) * (n - sz1) * (m - sz2) + Z(2) * cnt) * ksm(Z(2), ((n - 1) * (m - 1) % mod - k + mod) % mod);
