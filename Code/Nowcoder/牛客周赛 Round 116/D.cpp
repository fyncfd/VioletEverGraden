...
static inline void VioletEverGraden() {
    ...
    ll lmin = LINF; ll rmax = -LINF;
    ll lmax = -LINF; ll rmin = LINF;

    rep(i, 1, n, 1) {
        std::cin >> p[i].l >> p[i].r;
        lmin = std::min(lmin, p[i].l); rmax = std::max(rmax, p[i].r);
        lmax = std::max(lmax, p[i].l); rmin = std::min(rmin, p[i].r);
    }
    ...
    rep(i, 1, n, 1) {
        if (p[i].l == lmin) {
            lmax1 = std::max(lmax1, p[i].r);
        }
        if (p[i].r == rmax) {
            rmin1 = std::min(rmin1, p[i].l);
        }
    }
    ...
    rep(i, 1, n, 1) {
        ll x = p[i].l;
        ll y = p[i].r;
        if (x > lmin and y < lmax1) {
            std::cout << "No\n";
            return;
        }
        if (x > rmin1 and y < rmax) {
            std::cout << "No\n";
            return;
        }
    }
