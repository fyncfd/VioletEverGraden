...
* @Create Time: 2026-01-16 14:24:51
...
static inline void VioletEverGraden() {
    ...
    auto dfs1 = [&](auto &&dfs1, int x, int p) -> int {
    	if (x < 1LL << (2 * p - 1)) {
    		return p - 1;
    	}
    	return dfs1(dfs1, x - pow(2, 2 * p - 1), p + 1);
    };

    auto dfs2 = [&](auto &&dfs2, int x, int p) -> int {
    	if (x < 1LL << (2 * p)) {
    		return p;
    	}
    	return dfs2(dfs2, x - pow(2, 2 * p), p + 1);
    };


    int a1 = dfs1(dfs1, a, 1); int b2 = dfs2(dfs2, b, 0);
    int a2 = dfs2(dfs2, a, 0); int b1 = dfs1(dfs1, b, 1);
