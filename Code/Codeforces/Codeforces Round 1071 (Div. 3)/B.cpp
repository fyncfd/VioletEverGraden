...
* @Create Time: 2026-01-14 21:39:59
...
static inline void VioletEverGraden() {
    ...
    ll dif = std::abs(a[2] - a[1]);
    rep(i, 2, n - 1, 1) {
    	ll tmp = std::abs(std::abs(a[i + 1] - a[i]) + std::abs(a[i] - a[i - 1]) - std::abs(a[i + 1] - a[i - 1]));
    	dif = std::max(dif, tmp);
    }
    dif = std::max(dif, std::abs(a[n] - a[n - 1]));
