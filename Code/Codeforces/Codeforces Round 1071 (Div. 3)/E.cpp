...
* @Create Time: 2026-01-15 09:57:47
...
static inline void VioletEverGraden() {
    ...
    std::vector<ll> need(n + 1, 0);
    rep(i, 1, n, 1) {
    	need[i] = p[i] / 2 + 1;
    }

    ll sum = std::accumulate(p.begin() + 1, p.end() , 0LL);
    if (x + y < sum) {
    	std::cout << "NO\n";
    	return ;
    }
    ...
    if (s == " " + std::string(n, '1') or s == " " + std::string(n, '0')) {
    	if (s == " " + std::string(n, '1')) {
    		std::swap(x, y);
    	}

    	ll nx = std::accumulate(need.begin() + 1, need.end(), 0LL);
    	if (x < nx or x < y + n) {
    		std::cout << "NO\n";
    		return ;
    	} else {
