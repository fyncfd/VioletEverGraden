...
* @Create Time: 2026-01-17 22:37:01
...
static inline void VioletEverGraden() {
    ...
    rep(i, 1, n, 1) {
    	std::cin >> a[i].fi; a[i].se = 0;
    }

    std::vector<pii> p1 = a; std::vector<pii> p2 = a;
    rep(i, 1, n, 1) {
    	if (i & 1) {
    		p1[i].se = 1;
    		p2[i].se = 2;
    ...
    std::sort(p1.begin() + 1, p1.end(), [&](pii a, pii b) -> bool {
    	return a.fi < b.fi;
    });
    std::sort(p2.begin() + 1, p2.end(), [&](pii a, pii b) -> bool {
    	return a.fi < b.fi;
    });

    bool ok1 = false; bool ok2 = false;
    rep(i, 2, n, 1) {
    	if (p1[i].se == p1[i - 1].se) {
    		ok1 = true;
    	}
    	if (p2[i].se == p2[i - 1].se) {
    		ok2 = true;
    	}
    }
