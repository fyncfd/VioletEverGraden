...
* @Create Time: 2025-12-14 19:18:25
...
/*    for (auto x : a) {
    ...
    ll ans1 = 0; ll p1 = 0;
    if (a.size() & 1) {
    	p1 = (a[a.size() / 2] + a[a.size() / 2 + 1]) / 2;
    } else {
    	p1 = a[(a.size() - 1) / 2 + ((a.size() -1) % 2 != 0)];
    }
    rep(i, 1, a.size() - 1, 1) {
    	ans1 += std::abs(a[i] - p1);
    }
    ...
    ll ans2 = 0; ll p2 = 0;
    if (a.size() & 1) {
    	p2 = (b[b.size() / 2] + b[b.size() / 2 + 1]) / 2;
    } else {
    	p2 = b[(b.size() - 1) / 2 + ((b.size() -1) % 2 != 0)];
    }
    rep(i, 1, b.size() - 1, 1) {
    	ans2 += std::abs(b[i] - p2);
    }

    ll ans = std::min(ans1, ans2);
