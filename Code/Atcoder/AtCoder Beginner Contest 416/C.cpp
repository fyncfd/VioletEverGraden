...
* @Create Time: 2025-07-26 20:15:53
...
static inline void VioletEverGraden() {
    ...
    auto dfs = [&](auto &&self, int num, std::string ss) -> void {
    	if (num > k) {
    		ans.push_back(ss);
    		return ;
    	}
    	...
    	rep(i, 1, n, 1) {
            self(self, num + 1, ss + s[i]);
        }
    };

    dfs(dfs, 1, "");

    std::sort(ans.begin() + 1, ans.end(), [&](std::string a, std::string b) -> bool {
    	return a < b;
    });
