...
* @Create Time: 2025-11-22 20:42:24
...
static inline void VioletEverGraden() {
    ...
    ll res = 0;
    rep(i, 1, n, 1) {
        ll amod = a[i] % m; int lj = len[i];
        ll temp = (m - amod) % m; ll f = p10[lj];
        rep(j, 1, 10, 1) {
            auto it = hash[j].find(temp);
            if (it != hash[j].end() and (it->first * f) % m == temp) {
                res += it->second;
            }
        }
    }
