...
* @Create Time: 2026-01-13 22:38:32
...
static inline void VioletEverGraden() {
    ...
    rep(j, 3, N, 1) {
    	if (A[j] % 5 == 0) {
    		int cnt = A[j] / 5;
    		int Ai = cnt * 7; int Ak = cnt * 3;
    		if (hash1[Ai] and hash1[Ak]) {
    			ans += hash1[Ai] * hash1[Ak];
    		}
    		// debug(j);
    		// debug(Ai); debug(A[j]); debug(Ak);
    	}
    	hash1[A[j]] ++;
    }

    std::map<ll, ll> hash2;
    hash2[A[N]] ++; hash2[A[N - 1]] ++;
    dep(j, N - 2, 1, 1) {
    	if (A[j] % 5 == 0) {
    		int cnt = A[j] / 5;
    		int Ai = cnt * 7; int Ak = cnt * 3;
    		if (hash2[Ai] and hash2[Ak]) {
    			ans += hash2[Ai] * hash2[Ak];
    		}
    	}
    	hash2[A[j]] ++;
    }
