...
* @Create Time: 2025-07-28 23:01:21
...
static inline void VioletEverGraden() {
    ...
    auto f = [&](auto &&self, std::vector<int> cur) -> std::vector<int> {
        if (cur.size() == 1) {
            return cur;
        }

        std::vector<int> lft; std::vector<int> rft;
        int n = cur.size();
        rep(i, 0, n - 1, 1) {
            if (i < n / 2) {
                lft.push_back(cur[i]);
        ...
        auto ans1 = self(self, lft);
        auto ans2 = self(self, rft);
        if (ans1 < ans2) {
