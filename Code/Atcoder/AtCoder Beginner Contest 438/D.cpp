...
* @Create Time: 2026-01-14 16:57:21
...
static inline void VioletEverGraden() {
    ...
    std::vector<ll> preA(N + 1, 0); std::vector<ll> preB(N + 1, 0); std::vector<ll> sufC(N + 2, 0);
    rep(i, 1, N, 1) {
    	preA[i] = preA[i - 1] + A[i];
    	preB[i] = preB[i - 1] + B[i];
    }
    ...
    ll max = -LINF; ll ans = -LINF;
    rep(i, 2, N - 1, 1) {
    	max = std::max(max, preA[i - 1] - preB[i - 1]);
    	ll cur = max + preB[i] + sufC[i + 1];
    	ans = std::max(ans, cur);
    }
