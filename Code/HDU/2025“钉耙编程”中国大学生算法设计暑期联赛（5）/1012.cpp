...
* @Create Time: 2025-08-01 12:40:30
...
static inline void VioletEverGraden() {
    int n;
    cin >> n;
    vector<pair<long long, long long>> elements(n + 1); // Using 1-based indexing
    for (int i = 1; i <= n; ++i) {
    ...
    // Sort based on array a
    sort(elements.begin() + 1, elements.end());

    long long min_diff = LLONG_MAX;
    for (int i = 2; i <= n; ++i) {
        long long a_diff = elements[i].first - elements[i - 1].first;
        long long b_diff = abs(elements[i].second - elements[i - 1].second);
        long long current_diff = abs(a_diff - b_diff);
    ...
    // Sort based on array b
    sort(elements.begin() + 1, elements.end(), [](const pair<long long, long long>& x, const pair<long long, long long>& y) {
        return x.second < y.second;
    });

    for (int i = 2; i <= n; ++i) {
        long long a_diff = abs(elements[i].first - elements[i - 1].first);
        long long b_diff = elements[i].second - elements[i - 1].second;
        long long current_diff = abs(a_diff - b_diff);
        if (current_diff < min_diff) {
            min_diff = current_diff;
        }
