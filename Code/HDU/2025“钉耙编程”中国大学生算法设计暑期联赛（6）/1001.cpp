...
* @Create Time: 2025-08-04 16:11:07
...
static inline void VioletEverGraden() {
    ...
    if (neg % 2 != 0) {
        if (zero > 0) {
            // one of the zeros was turned to -1, now we can switch one -1 to 1, adding 2 operations (since -1 to 1 is 2 steps)
            total_ops += 2;
        } else {
            // no zeros, need to change one -1 to 1, which costs 2 operations (from -1 to 1)
            total_ops += 2;
        }
    }
    cout << total_ops << endl;
